...
%token BEGIN END CYCLE ASSIGN ASSIGNPLUS ASSIGNMINUS ASSIGNMULT SEMICOLON WRITE VAR PLUS MINUS MULT DIV LPAREN RPAREN COLUMN IF THEN ELSE COLON GOTO 
%token <iVal> INUM 
%token <dVal> RNUM 
%token <sVal> ID 

%type <eVal> expr ident T F 
%type <stVal> statement assign block cycle write empty var varlist if label goto
...
label   : ident COLON statement { $$ = new labelNode($1 as IdNode, $3); }
		;
